import React, { Component } from "react";
import "./App.scss";
import MapPage from "./containers/MapPage";
import Tooltip from "./containers/components/Tooltip";

export default class App extends Component {
  constructor(props) {
    super(props);
    this.state = {
      style: "mapbox://styles/paugmnoz/ck6wpmuxt0v4i1it6mxplyle5"
    };
  }
  changeStyle(style, event) {
    console.log(event.currentTarget, 'elemento')
    let configStyle = "mapbox://styles/paugmnoz/" + style;
    this.setState({
      style: configStyle
    });
  }
  render() {
    return (
      <div className="App">
        <MapPage mapStyle={this.state.style} />
        <div id="map-title">
          <span className="title-texts">
            <h2>Traffic Accidents Hotspots</h2>
            <h4>Dallas and Fort Worth</h4>
          </span>
          <span className="accidents-text">
            <p>TOTAL ACCIDENTS</p>
            <h1>32506</h1>
          </span>
        </div>
        <div id="light-menu">
          <h4>Light conditions</h4>
          <article
            className="light-btn"
            onClick={(e) => this.changeStyle("ck6wpmuxt0v4i1it6mxplyle5", e)}
          >
            <p>All</p>
          </article>
          <article
            className="light-btn active"
            onClick={(e) => this.changeStyle("ck759xrao0djd1ippru4diusr", e)}
          >
            <img src="./filter-icons/active/dawn.svg" alt="" />
            <p>Dawn</p>
          </article>
          <article
            className="light-btn active"
            onClick={() => this.changeStyle("ck75an5yx1vtd1ikpu9x21glf", this)}
          >
            <img src="./filter-icons/active/daylight.svg" alt="" />
            <p>Daylight</p>
          </article>
          <article
            className="light-btn active"
            onClick={() => this.changeStyle("ck75af3ax10p81ilekna2gcoc", this)}
          >
            <img src="./filter-icons/active/dusk.svg" alt="" />
            <p>Dusk</p>
          </article>
          <section className="dark-sec">
            <h4>Dark</h4>
            <article
              className="light-btn active"
              onClick={() => this.changeStyle("lighted", this)}
            >
              <img src="./filter-icons/active/lighted.svg" alt="" />
              <p>Lighted</p>
            </article>
            <article
              className="light-btn active"
              onClick={() => this.changeStyle("nolighted",this)}
            >
              <img src="./filter-icons/active/not-lighted.svg" alt="" />
              <p>Not lighted</p>
            </article>
            <article
              className="light-btn active"
              onClick={() => this.changeStyle("dark", this)}
            >
              <img src="./filter-icons/active/unknown-light.svg" alt="" />
              <p>Unknown</p>
            </article>
          </section>
        </div>
       
      </div>
    );
  }
}
